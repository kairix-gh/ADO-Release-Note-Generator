    ...
    container.Page(page => {
        ...
        page.Content().PaddingVertical(1, Unit.Centimetre).Column(x => {
            ...
            foreach (var i in stories) {
                if (!i.Fields.TryGetValue("Custom.ReleaseNotesNotes", out content)) {
                    i.Fields.TryGetValue("System.Title", out content);
                }
                ...
                if (content != string.Empty) {
                    x.Item().PaddingBottom(10).Row(row => {
                        ...
                        row.RelativeItem().Column(col => {
                            ...
                            col.Item().Text(Placeholders.LoremIpsum()).FontSize(12).FontColor(Colors.Grey.Darken3);
                        });
                    });
                    count++;
                }
